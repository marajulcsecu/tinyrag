...
**Last updated:** 2026-06-24 (update 28)
**Project status:** Step 4.9 complete — 🛑 RISK GATE cleared: `scripts/ingest.py` walks a real PDF through parse → chunk → embed → store end-to-end; the 4-stage pipeline produces an IngestionReport in < 5 seconds on the laptop
**Next milestone:** Step 4.10 — Implement the LLM client (`LlamaCppClient` over llama-server's HTTP API with streaming)
...
**Tip of `main`:** `ee66403` (see §11 Build Journal)
...
- ⏳ Next: Step 4.10 — Implement the LLM client (`LlamaCppClient` over llama-server's HTTP API with streaming)

**Immediate next step (Step 4.10 — agent action):**

Step 4.10 implements the **generation** half of RAG (so far Phase 4 has built the ingestion half — parser, chunker, embedder, both stores, end-to-end script). It writes `src/tinyrag/generation/llm_client.py` with the `LLMClient` Protocol (already shipped from Step 3.7), the concrete `LlamaCppClient` (streaming `generate(prompt, max_tokens, temperature) -> Iterator[str]` via `httpx.Client.stream()` for SSE), plus `model_name()` and `is_healthy()` introspection methods. The typed exception hierarchy (`LLMError` → `LLMConnectionError`, `LLMTimeoutError`, `LLMProtocolError`) lets the API layer (Step 4.13) map failures to clean 503 / 504 responses. Plus `tests/test_llm_client.py` with mocked HTTP responses (the existing file may already cover this — re-evaluate). After this step, the project has **both halves** of RAG and can start building the prompt + retriever on top.

**Optional parallel student action — you can verify Step 4.9 yourself with these commands (the FAKE embedder path is hermetic — no model download required):**

```bash
# 1. Verify the ingest pipeline against the Nest PDF (already in tests/fixtures/)
#    using the FakeEmbedder (no model download, ~3 s end-to-end)
PYTHONPATH=src ~/venvs/tinyrag/bin/python scripts/ingest.py \
    tests/fixtures/Nest-Thermostat-Installation-Guide-UK.pdf \
    --embedder fake
...
#   index_size:          44
#   TOTAL:            ~2050 ms
# [ OK ] Ingestion succeeded — chunks are now searchable.

# 2. Same thing in JSON (for piping to other tools / CI)
PYTHONPATH=src ~/venvs/tinyrag/bin/python scripts/ingest.py \
    tests/fixtures/Nest-Thermostat-Installation-Guide-UK.pdf \
    --embedder fake --json | head -20

# 3. Visual confirmation — open the SQLite DB in DB Browser for SQLite
#    (or sqlite3 CLI) and confirm the document + 44 chunks are there
sqlite3 data/metadata.db "SELECT filename, num_chunks FROM documents;"
