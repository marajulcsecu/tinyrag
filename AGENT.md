...
**Last updated:** 2026-06-27 (update 38)
**Project status:** Step 4.18 complete — Document management HTTP endpoints replacing the Step 4.17 501 skeletons: `POST /api/documents` (multipart upload → `tempfile.NamedTemporaryFile` → `scripts.ingest.run_ingest` → `MetadataStore.update_document_provenance` to overwrite the ingest-script's tempfile-name with the user-supplied basename → `doc_store.load()` so new vectors are queryable immediately; validates extension `{.pdf,.txt,.md}` + size ≤50 MiB + `doc_type` against the canonical `MetadataStore.SUPPORTED_DOC_TYPES`; rejects `.csv` etc with 400 `unsupported_file_type`, oversize with 413 `file_too_large`, missing `file` part with 422; returns `IngestionReport.to_dict()` with 200 on `ok` / 400 on failure), `GET /api/documents?limit&offset` (paginated list with `next_offset` cursor; `limit` clamped to `[1, 500]`, `offset` to `[0, ∞)`), `DELETE /api/documents/{id}` (cascade-delete: `doc_store.remove_ids(chunks)` then `metadata.delete_document` then `doc_store.save`; returns precise `chunks_removed` / `vectors_removed` counts; 404 when the id is unknown). Validation constants surfaced as module exports: `ALLOWED_EXTENSIONS`, `MAX_UPLOAD_BYTES` (50 MiB per FR-10), `SUPPORTED_DOC_TYPES`, `DEFAULT_DOC_TYPE="manual"`, `_safe_filename` (strips POSIX + Windows path separators + Windows-reserved device names `CON`/`PRN`/`AUX`/`NUL`/`COM1-9`/`LPT1-9`). `MetadataStore.list_documents(limit=None, offset=0)` extended (back-compat defaults preserved). New `MetadataStore.update_document_provenance(document_id, *, filename=None, source_path=None)` for the post-ingest provenance overwrite. New Pydantic schemas `DocumentListItemResponse` + `DocumentListResponse` + `DocumentDeleteResponse` in `tinyrag/api/schemas.py`; `DocumentUploadResponse = dict[str, Any]` re-uses the `IngestionReport.to_dict()` shape. `scripts/ingest.run_ingest` is imported via a localised `sys.path` bootstrap (mirroring the test-suite pattern) so we don't have to refactor the CLI script into the package for Step 4.18. Admin endpoints (`/api/admin/reindex`, `/api/admin/benchmark`) remain 501 — they're Phase 5 work. Plus 30 new tests in `tests/test_api.py` (4 classes: TestDocsRouterExports, TestPostDocuments, TestGetDocuments, TestDeleteDocument) + 3 pagination tests in `tests/test_metadata.py`. Full suite **1138 passed, 8 skipped** (+30 new tests, 0 regressions). Lint clean (`ruff check src/tinyrag/ tests/` → 0 errors).
**Next milestone:** Step 4.19 — SSE streaming for `/api/query`: replace the synchronous JSON response with `sse_starlette.EventSourceResponse`, emit `data: {"event": "token", "delta": "..."}` events as the LLM streams, plus `data: {"event": "done", "answer": ...}` at the end. The route handler shape stays almost identical — we just swap the return type. Tests need to verify the SSE event stream + that `used_sensor_idx` + `top_score` still flow through the final `done` event. The dashboard's chat panel (Step 4.21) gets true token-by-token rendering.
...
**Tip of `main`:** `13a272c` (see §11 Build Journal)
...
**Immediate next step (Step 4.19 — agent action):**

Step 4.19 turns `POST /api/query` into a streaming endpoint: instead of waiting for the LLM to finish then returning one JSON blob, the route handler opens an SSE channel (`sse_starlette.EventSourceResponse`) and emits `data: {"event": "token", "delta": "..."}` events as each token arrives from the LLM, plus a final `data: {"event": "done", "answer": {…}}` carrying the full `Answer.to_dict()` shape (citations, durations, used_sensor_idx, top_score — everything the current synchronous response carries). The non-streaming route stays available behind a `?stream=true|false` query param so existing CLI / curl users don't break. The handler shape stays almost identical to Step 4.17 — we swap the return type and wrap each LLM token in an SSE event. The dashboard's chat panel (Step 4.21) gets true token-by-token rendering. Tests will need a small `sse_starlette` TestClient adapter to parse the event stream + assert that `used_sensor_idx` + `top_score` flow through the final `done` event.

**Optional parallel student action — you can verify Step 4.18 yourself:**

```bash
# 1. Run the new test suite (covers the POST/GET/DELETE endpoints,
#    extension/size/doc_type rejection, filename sanitisation,
#    cascade-delete with chunks_removed / vectors_removed counts,
#    + the 3 new MetadataStore pagination tests). Expect 33 new
#    passed in ~10 s.
PYTHONPATH=src ~/venvs/tinyrag/bin/python -m pytest \
    tests/test_api.py::TestDocsRouterExports \
    tests/test_api.py::TestPostDocuments \
    tests/test_api.py::TestGetDocuments \
    tests/test_api.py::TestDeleteDocument \
    tests/test_metadata.py::TestListDocuments -v

# 2. Start the FastAPI server against the real config and probe
#    the new endpoints. Pick any free port (8765 used here).
#    Use --log-level warning so the structured log doesn't drown
#    the curl output.
PYTHONPATH=src ~/venvs/tinyrag/bin/python -m uvicorn \
    tinyrag.main:app --host 127.0.0.1 --port 8765 --log-level warning &
sleep 4

# 3. Upload the real Nest PDF (uses the same fixture the
#    ingest pipeline tests use):
curl -s -X POST http://127.0.0.1:8765/api/documents \
    -F 'file=@tests/fixtures/Nest-Thermostat-Installation-Guide-UK.pdf' \
    -F 'doc_type=manual' | python -m json.tool
# Expected: 200, ok=true, doc_id is a UUID, num_chunks=44,
# duration_total_ms ~ 2.5 s. The "file" field shows the
# tempfile path used internally — the user-supplied filename is
# recorded in the metadata row (verified in the GET below).

# 4. List the documents — the uploaded doc appears with the
#    user-supplied basename:
curl -s http://127.0.0.1:8765/api/documents | python -m json.tool
# Expected: count=1, documents[0].filename="Nest-Thermostat-Installation-Guide-UK.pdf",
# documents[0].num_chunks=44.

# 5. Delete the doc:
DOC_ID=$(curl -s http://127.0.0.1:8765/api/documents \
    | python -c "import sys, json; print(json.load(sys.stdin)['documents'][0]['id'])")
curl -s -X DELETE http://127.0.0.1:8765/api/documents/$DOC_ID \
    | python -m json.tool
# Expected: 200, chunks_removed=44, vectors_removed=44.

# 6. List again — empty:
curl -s http://127.0.0.1:8765/api/documents
# Expected: count=0, documents=[].

# 7. Try the validation paths (every one returns the uniform
#    ErrorResponse JSON shape):
echo "test" > /tmp/notes.txt
curl -s -X POST http://127.0.0.1:8765/api/documents \
    -F 'file=@/tmp/notes.txt' \
    -F 'doc_type=bogus_type'
# Expected: {"error":"invalid_doc_type","detail":"doc_type 'bogus_type' not in [...]"}
curl -s -X POST http://127.0.0.1:8765/api/documents \
    -F 'file=@/etc/hosts;filename=evil.csv'
# Expected: {"error":"unsupported_file_type","detail":"extension '.csv' not in [...]"}
curl -s "http://127.0.0.1:8765/api/documents?limit=501"
# Expected: 422 (Pydantic Query bounds; uniform error shape).
curl -s -X DELETE http://127.0.0.1:8765/api/documents/00000000-0000-0000-0000-000000000000
# Expected: {"error":"document_not_found","detail":"no document with id '00000000-...'"}.

# 8. Restart-persistence check: kill uvicorn, restart, list — the
#    delete from step 5 must still be reflected:
kill %1 2>/dev/null; sleep 2
PYTHONPATH=src ~/venvs/tinyrag/bin/python -m uvicorn \
    tinyrag.main:app --host 127.0.0.1 --port 8765 --log-level warning &
sleep 4
curl -s http://127.0.0.1:8765/api/documents
# Expected: count=0 (SQLite + FAISS sidecar were both persisted).
kill %1 2>/dev/null
```
