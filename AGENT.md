...
**Last updated:** 2026-06-24 (update 22)
**Project status:** Step 4.4 complete — document parsers (PDF/TXT/MD) live; the ingestion pipeline can now turn any uploaded file into structured text
**Next milestone:** Step 4.5 — Implement the chunker (token-based with overlap)
...
**Tip of `main`:** `29e2810` (see §11 Build Journal)
...
- ⏳ Next: Step 4.5 — Implement the chunker (token-based with overlap)

**Immediate next step (Step 4.5 — agent action):**

Step 4.5 fills `src/tinyrag/core/chunker.py` with a `Chunker` class that takes a `ParsedDocument` and produces a list of `Chunk` dataclasses, using `tiktoken` (cl100k_base) for token counting. Splits at 400 tokens with 50-token overlap, respects sentence boundaries when possible. This is the bridge between "text from a file" (parsers) and "vectors in the store" (embedder + vector store). Per FR-3 + FR-5: each chunk carries its source filename, page number (for PDFs), chunk index, character offset, and ingestion timestamp.

**Optional parallel student action — none for Step 4.5. You can verify the Step 4.4 parsers yourself with:**

```bash
# 1. Run the parser tests (56 should pass)
PYTHONPATH=. .venv/bin/pytest tests/test_parsers.py -v

# 2. Quick REPL demo — parse a real .txt / .md / .pdf file.
#    If you don't have a PDF handy, build one inline:
...
p.write_text('Hello from the TinyRAG parser.\nLine two.\n', encoding='utf-8')
doc = parse(p)
print('format:', doc.metadata['format'])
print('text:', repr(doc.text))
print('pages:', doc.pages)
print('metadata:', doc.metadata)
"

# 3. Same for Markdown — front-matter and link URLs are stripped automatically:
PYTHONPATH=src .venv/bin/python -c "
from pathlib import Path
from tinyrag.ingestion.parsers import parse
...
The first command runs the test suite (which exercises the hand-built PDF fixture too). The second prints the extracted text and metadata for a TXT — note that `metadata['format']` is `"txt"`, `pages` is a one-element list, and the `metadata` dict is JSON-serialisable (so it can flow straight into SQLite in Step 4.7). The third shows the Markdown parser stripping the YAML front-matter and reducing `[docs](url)` to just `docs`.
...
| Step | Description | Status | Commit SHA | Commit message | Notes |
|------|-------------|--------|------------|----------------|-------|
| 4.5 | Implement the chunker | ⏳ Next | — | — | Will fill `src/tinyrag/core/chunker.py` with `Chunker.chunk(text, source, page)` using `tiktoken` (cl100k_base). Splits at ~400 tokens with 50-token overlap, respects sentence boundaries. Defines `Chunk` dataclass (`text`, `source`, `page`, `chunk_index`, `char_offset`, `token_count`) satisfying FR-3 + FR-5 metadata requirements. |
