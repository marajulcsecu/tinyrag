...
**Last updated:** 2026-06-24 (update 29)
**Project status:** Step 4.10 complete — `LlamaCppClient` over llama-server's HTTP API with SSE streaming is live and verified end-to-end (tinyllama-1.1b smoke test: 25 tokens in 2.91 s @ 8.58 tok/s); the API `/health` endpoint can now use `is_healthy()` to probe server reachability
**Next milestone:** Step 4.11 — Implement the prompt builder (grounded prompt from system instructions + retrieved chunks + query, fits in 4096-token context)
...
**Tip of `main`:** `168ec90` (see §11 Build Journal)
...
- ⏳ Next: Step 4.11 — Implement the prompt builder (grounded prompt from system instructions + retrieved chunks + query, fits in 4096-token context)

**Immediate next step (Step 4.11 — agent action):**

Step 4.11 writes `src/tinyrag/core/prompt_builder.py` — the function that takes a query + retrieved chunks and emits a single, well-structured prompt. This is where the "grounded answer" guarantee is enforced: the system prompt instructs the LLM to cite `[1]`/`[2]`/`[3]` style chunks and refuse if the answer isn't in the context; the user-message body is `[1] <chunk 1 text> \n [2] <chunk 2 text> \n ... \n Question: <query>`. The function must (a) count tokens as it builds (use the chunker's tiktoken encoding) so we can refuse / truncate if we'd blow past 4096, and (b) have well-defined behaviour for 0 chunks (refuse — don't make stuff up), 1 chunk, max chunks, and very-long chunks (truncate mid-sentence). This step unlocks Step 4.12 (retriever → prompt → LLM) so we can finally answer a question end-to-end.
