...
**Last updated:** 2026-06-27 (update 35)
**Project status:** Step 4.15 complete — `scripts/ingest_sensors.py` is the producer-side ingest CLI for the sensor path: SensorSource → DataFrame → SensorSummarizer → embedder → a SEPARATE sensor FAISS index + a `documents` row with `doc_type='sensor_summary'`. 5-stage pipeline (read, summarize, embed, metadata, vector) with per-stage timing; idempotent re-ingest via filename + `doc_type` (clears prior chunks + FAISS slots before re-adding); CLI flags `--csv`, `--source {simulated|mqtt|real_serial}`, `--since`, `--embedder {real|fake}`, `--force`, `--json`, `--quiet`. Phase 6 stubs (`real_serial`, `mqtt`) surface a clean `SensorSourceError`. Real CSV → 180 chunks in ~800 ms (FakeEmbedder); FAISS index size matches chunk count. Full suite **984 passed, 8 skipped** (+80 new tests, 0 regressions)
**Next milestone:** Step 4.16 — 🛑 RISK GATE: end-to-end RAG via CLI (`scripts/ask.py "How do I reset my thermostat?"` returns a cited answer; same pipeline as Step 4.12 retriever + Step 4.11 prompt builder, but in a CLI script)
...
**Immediate next step (Step 4.16 — agent action):**

Step 4.16 wires the full query loop on top of every Phase 4 producer: `scripts/ask.py` — the end-to-end **question-answering CLI** (and the second RISK GATE). Reads a user question from argv → `Retriever.retrieve(question)` (sensor-keyword routing + two-store merge) → `PromptBuilder.build(question, chunks)` (token-budget-aware, citations) → `LLMClient.stream_chat(prompt)` (SSE over llama-server) → writes the answer to stdout. Also wires the structured `QueryLog` row into `MetadataStore` so the eval set can later grade answer quality. After Step 4.16, the system answers real questions end-to-end on the laptop (no notebook required). The hardware RISK GATE (Pi 5 + llama.cpp) is then Phase 5.
