...
> - llama.cpp build flags in Section 14 use OpenBLAS (`-DGGML_OPENBLAS=ON`).
> - Sensor source defaults to `SimulatedCSVSource` (no GPIO on laptop).
> - Performance expectations match the laptop column of the SRS NFR table.
>
> **Identical content otherwise.** See `docs/03_architecture_v1.md` for the canonical version. If the two diverge, the canonical version wins.
