...
> - Performance targets (Section 4) use the **laptop column** instead of the Pi 5 column.
> - The deployment target in Appendix B defaults to `laptop`.
> - Real GPIO sensor sources (FR-11b, `RealSerialSource`) are **not applicable** on a laptop — use `SimulatedCSVSource` or `MQTTBrokerSource` instead.
>
> **Identical content otherwise.** See `docs/02_srs_v1.md` for the canonical version. If the two diverge, the canonical version wins.
